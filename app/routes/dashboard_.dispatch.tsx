...
import DispatchMapComponent from "~/components/DispatchMapComponent";
    ...
    >
      ...
      <SidebarInset className={isInsetVariant ? "rounded-lg shadow-lg m-2 ml-0 overflow-hidden" : ""}>
        <div className={`relative ${isInsetVariant ? "h-[calc(100vh-1rem)]" : "h-screen"}`}>
          <ClientOnly fallback={<div className="h-screen w-full bg-gray-100 flex items-center justify-center">Cargando asistente de despacho...</div>}>
            {() => <DispatchMapComponent isInsetVariant={isInsetVariant} setIsInsetVariant={setIsInsetVariant} />}
          </ClientOnly>
        </div>
      </SidebarInset>
